...
void csv_read_cell(CSVFILE *data, size_t row, size_t col, char *value) {
  if (row <= data->number_of_rows && col <= data->number_of_columns) {
    ...
    printf("Cell Number (%zd, %zd) of length (%zd) : ", row, col, len);
    for (int i = data->cell_start[cell_number];
         i < data->cell_start[cell_number + 1] - 1; ++i)
      printf("%c", data->file_string[i]);
    printf("\n");
  }
}
